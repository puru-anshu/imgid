...
public class FFTN {
    ...
    public FFTN(int length) throws IllegalArgumentException {
     ...
     * during the fft.
     */
    public void fft2(double[] re, double[] im) {
        butterfly.fft(re, im);
    }
     ...
     */
    public static double[] PowerSpectrum(double[] re, double[] im) {
        double[] power = new double[re.length / 2 + 1];
        int i = 0;
     ...
     */
    public static double[] PowerSpectrum(double[] re) {
        double[] power = new double[re.length / 2 + 1];
        int i = 0;
        for (; i < power.length; ++i)
            power[i] += re[i] * re[i];
    ...
    private static double[] wRe(int length) {
        double[] w_re = new double[length];
        for (int i = 0; i < w_re.length; ++i)
            w_re[i] = (double) Math.cos(2.0 * Math.PI * i / length);
        return w_re;
    }

    private static double[] wIm(int length) {
        double[] w_im = new double[length];
        for (int i = 0; i < w_im.length; ++i)
            w_im[i] = (double) -Math.sin(2.0 * Math.PI * i / length);
        return w_im;
    }

    private void perfectShuffle(double[] src,
                                double[] dest,
                                int blocksize) {
        int i, i_max, j, k;
    ...
    private class Butterfly2n
        ...
        public void fft(double[] re, double[] im) {
            double[] tmp_re = new double[re.length];
            double[] tmp_im = new double[im.length];
         ...
         * re = [ r_0, r_1, ..., r_(length/2), i_(length/2-1), ..., i_1 ]
         */
        public void fft(double[] re) {
            double[] tmp_re = new double[re.length / 2];
            double[] tmp_im = new double[re.length / 2];
            ...
            for (i = 1, ii = 2 + (j = tmp_re.length - 1), jj = re.length - 1;
                ...
                double tmp1, tmp0 = w_re[i] * ip + w_im[i] * rm;
                re[i] = rp + tmp0;
                re[j] = rp - tmp0;
    ...
    private class Butterfly4
        ...
        public void fft(double[] re) {
            for (int l = 0, l1, l2, l3; l < re.length; l += 4) {
                l3 = 1 + (l2 = 1 + (l1 = 1 + l));
                double tmp_0 = re[l] + re[l2];
                double tmp_2 = re[l1] + re[l3];
                re[l3] -= re[l1];
                re[l1] = re[l] - re[l2];
                re[l] = tmp_0 + tmp_2;
                re[l2] = tmp_0 - tmp_2;
    ...
    private class Butterfly2
        ...
        public void fft(double[] re, double[] im) {
            for (int l = 0, l1 = 1; l < re.length; l1 = 1 + (l += 2)) {
                double real = re[l] - re[l1];
                re[l] += re[l1];
                double imag = im[l] - im[l1];
                im[l] += im[l1];
                re[l1] = real;
                im[l1] = imag;
            }
    ...
    /*
      public static void main(String[] args) {
        ...
        double[] re = new double[length*2];
        double[] im = new double[length*2];
    ...
    */
    ...
    private static void fou(double data[], int nn)

    {
        ...
        while (n > mmax) {
            istep = mmax << 1;
            theta = (double) 6.28318530717959 / mmax;
    ...
    public void fft(double re[]) {
        ...
        double c1 = (double) 0.5, c2 = (double) -0.5, h1r, h1i, h2r, h2i;
        double wr, wi, wpr, wpi, wtemp, theta;
        for (i = n; --i >= 0;) data[i + 1] = re[i];
        theta = (double) 3.141592653589793 / (double) (n >>> 1);
