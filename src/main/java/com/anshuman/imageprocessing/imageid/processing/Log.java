...
public class Log {
    ...
    public static void main(String[] args) throws Exception {
        String pathname = "e:\\Lena.png";
        Image fromArray = getLogImage(pathname);
        JOptionPane.showMessageDialog(null, new ImageIcon(fromArray));
    }

    public static Image getLogImage(String pathname) throws IOException {
        BufferedImage bufferedImage = ImageIO.read(new File(pathname));
        int[] ints = ImageOperation.getImagePixels(bufferedImage);
        Log log = new Log(7, 0.55);
        int w = bufferedImage.getWidth();
        int h = bufferedImage.getHeight();
        int[] ints1 = log.log_image(ints, w, h, 1.0f, 0);
        Image fromArray = ImageOperation.getImageFromArray(ints1, w, h);
        return fromArray;
    }
